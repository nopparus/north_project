...
export default function App() {
  ...
  const [holidayImportModal, setHolidayImportModal] = useState<{ isOpen: boolean; holidays: ThaiHoliday[]; data: any } | null>(null);
  ...
  useEffect(() => {
    const loadServerData = async () => {
      try {
        const [serverProfiles, serverWages, serverHolidays] = await Promise.all([
        ...
        // Holidays Sync Logic
        if (serverHolidays.length === 0 && customHolidays.length > 0) {
          await holidayService.sync(customHolidays);
          const synced = await holidayService.getAll();
          setCustomHolidays(synced);
        } else if (serverHolidays.length > 0) {
          setCustomHolidays(serverHolidays);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (isLoaded) {
      const config = {
        ...
        settings,
        rows,
        // customHolidays: removed from persistence — now server-side
        // wages: removed from persistence — now server-side
        activeTab,
        isDarkMode
        // profiles: removed from persistence — now server-side
      };
      localStorage.setItem('security_budget_config', JSON.stringify(config));
    }
  }, [startDate, endDate, settings, rows, wages, activeTab, isDarkMode, isLoaded]);
  ...
  const confirmExport = () => {
    ...
    const config = {
      ...
      customHolidays: customHolidays.filter(h => {
        ...
        const standardHolidays = DEFAULT_THAI_HOLIDAYS(year);
        // Only keep holidays that are NOT in the standard list
        return !standardHolidays.some(sh => sh.date === h.date);
      }),
    ...
    a.href = url;
    a.download = `${exportModal.filename || 'security-budget'}.mok`;
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.toLowerCase().endsWith('.mok')) {
      alert('ไฟล์ไม่ถูกต้อง กรุณาใช้ไฟล์นามสกุล .mok เท่านั้น');
      e.target.value = '';
      return;
    }
  ...
  const applyImport = (mode: 'overwrite' | 'merge') => {
    ...
    // Filter out standard holidays from data.customHolidays if it exists
    const fileHolidays = data.customHolidays || [];
    const specialHolidays = fileHolidays.filter((h: ThaiHoliday) => {
      const year = new Date(h.date).getFullYear();
      const standardHolidays = DEFAULT_THAI_HOLIDAYS(year);
      return !standardHolidays.some(sh => sh.date === h.date);
    });
    ...
    if (specialHolidays.length > 0) {
      setHolidayImportModal({
        isOpen: true,
        holidays: specialHolidays,
        data: { ...data, importMode: mode }
      });
      setImportModal(null); // Close the initial import modal
    } else {
      executeApplyImport(data, mode);
    }
  };

  const executeApplyImport = (data: any, mode: 'overwrite' | 'merge') => {
    if (mode === 'overwrite') {
      if (data.startDate) setStartDate(data.startDate);
      if (data.endDate) setEndDate(data.endDate);
      if (data.settings) setSettings(data.settings);
      ...
      if (data.customHolidays) {
        // Merge file holidays with standard ones for the range
        // This ensures we get all standard holidays plus the imported ones, without duplicates
        const startY = parse(data.startDate || startDate, 'yyyy-MM', new Date()).getFullYear();
        const endY = parse(data.endDate || endDate, 'yyyy-MM', new Date()).getFullYear();

        const mergedHolidays: ThaiHoliday[] = [];
        const seenDates = new Set<string>();

        // 1. Add standard holidays first
        for (let y = startY; y <= endY; y++) {
          DEFAULT_THAI_HOLIDAYS(y).forEach(sh => {
            mergedHolidays.push(sh);
            seenDates.add(sh.date);
          });
        }

        // 2. Add imported holidays, overriding standard ones if date matches
        data.customHolidays.forEach((h: ThaiHoliday) => {
          const idx = mergedHolidays.findIndex(mh => mh.date === h.date);
          if (idx >= 0) {
            mergedHolidays[idx] = h;
          } else {
            mergedHolidays.push(h);
          }
        });

        setCustomHolidays(mergedHolidays.sort((a, b) => a.date.localeCompare(b.date)));
      }
      if (data.wages) setWages(data.wages);
      if (data.profiles) setProfiles(data.profiles); // Overwrite profiles
    } else {
      // Merge logic
      if (data.rows) {
      ...
      if (data.customHolidays) {
        setCustomHolidays(prev => {
          ...
          // 1. Add existing holidays if merging, or start fresh if overwriting (handled by caller logic)
          if (mode === 'merge') {
            prev.forEach(h => holidayMap.set(h.date, h));
          }

          // 2. Add imported holidays (they override existing ones on same date)
          data.customHolidays.forEach((h: ThaiHoliday) => holidayMap.set(h.date, h));
  ...
  const confirmHolidayImport = async (shouldSync: boolean) => {
    if (!holidayImportModal) return;
    const { data, holidays } = holidayImportModal;
    const mode = data.importMode;

    if (shouldSync) {
      if (!isAuthenticated) {
        setShowLoginModal(true);
        return; // Stay on the holiday import modal
      }
      try {
        await holidayService.sync(holidays);
      } catch (error) {
        console.error("Failed to sync holidays", error);
        alert('ไม่สามารถบันทึกวันหยุดลงฐานข้อมูลได้');
      }
    }

    executeApplyImport(data, mode);
    setHolidayImportModal(null);
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      setCustomHolidays(prev => {
        ...
        for (let y = startY; y <= endY; y++) {
          DEFAULT_THAI_HOLIDAYS(y).forEach(sh => {
            if (!holidayMap.has(sh.date)) {
              holidayMap.set(sh.date, sh);
              changed = true;
            }
          });
        }
  ...
  // Per-weekday holiday count (from customHolidays filtered to range)
  const holidaysByDayOfWeek = useMemo(() => {
    ...
    customHolidays.forEach(h => {
      ...
      if (d >= startOfMonth(start) && d <= end) {
        counts[d.getDay()]++;
      }
    });
    return counts;
  }, [customHolidays, startDate, endDate]);
  ...
  const updateProvinceWage = async (province: string, wage: number) => {
    ...
    const confirmed = await confirmAction({
      title: 'ยืนยันบันทึกค่าแรง',
      message: `บันทึกค่าแรงขั้นต่ำเป็น ${wage} บาท/วัน สำหรับจังหวัด${province} ลง database (ใช้ร่วมกันทุกคน) ใช่หรือไม่?`
    });
    if (!confirmed) return;
    try {
  ...
  // Auto-sync rows minWage when wages state changes (fix: province wage update reflects immediately)
  React.useEffect(() => {
    setRows(prev => prev.map(r => {
      if (r.province && wages[r.province]) {
        return { ...r, minWage: wages[r.province] };
      }
      return r;
    }));
  }, [wages]);
  ...
  const toggleHoliday = async (date: string) => {
    ...
    if (exists) {
      ...
      if (confirmed) {
        try {
          await holidayService.delete(date);
  ...
  const confirmAddHoliday = async () => {
    if (holidayModal) {
      try {
        await holidayService.save(holidayModal.date, holidayModal.name);
        setCustomHolidays(prev => [...prev, { date: holidayModal.date, name: holidayModal.name }]);
        setHolidayModal(null);
      } catch {
        alert('ไม่สามารถบันทึกวันหยุดได้');
      }
    }
  };
  ...
  const resetHolidays = async () => {
    ...
    if (confirmed) {
      ...
      try {
        await holidayService.sync(newHolidays);
        setCustomHolidays(newHolidays);
      } catch {
        alert('ไม่สามารถรีเซ็ตวันหยุดได้');
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans pb-20 transition-colors duration-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 no-print">
        {activeTab === 'budget' ? (
          <>
            ...
            <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                  {/* 7 Day Breakdown — compact */}
                  <div className="grid grid-cols-7 divide-x divide-slate-100 dark:divide-slate-700/50">
                    {['อาทิตย์', 'จันทร์', 'อังคาร', 'พุธ', 'พฤหัส', 'ศุกร์', 'เสาร์'].map((day, i) => {
                      const total = periodStats?.dayOfWeekCounts[i] || 0;
                      const isWeekend = i === 0 || i === 6;
                      const holidays = isWeekend ? total : (holidaysByDayOfWeek[i] || 0);
                      const workDays = total - holidays;
                      return (
                        <div key={day} className={cn(
                          "flex flex-col items-center py-2.5 px-1",
                          isWeekend ? "bg-red-50/50 dark:bg-red-900/10" : "bg-white dark:bg-slate-900"
                        )}>
                          <span className={cn(
                            "text-[16px] font-bold mb-1.5 whitespace-nowrap",
                            isWeekend ? "text-red-500 dark:text-red-400" : "text-slate-400 dark:text-slate-500"
                          )}>{day}</span>
                          {/* Single-line: work / holiday */}
                          <div className="flex items-center text-sm font-bold leading-none">
                            <span className={cn(isWeekend ? "text-slate-300 dark:text-slate-600" : "text-slate-700 dark:text-slate-200")}>
                              {workDays > 0 ? workDays : '–'}
                            </span>
                            {holidays > 0 && (
                              <>
                                <span className="text-slate-300 dark:text-slate-600 mx-0.5">/</span>
                                <span className="text-red-500 dark:text-red-400">{holidays}</span>
                              </>
                            )}
                          </div>
                        </div>
                  ...
                  {/* Summary row */}
                  <div className="grid grid-cols-4 divide-x divide-slate-100 dark:divide-slate-700/50 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/30">
                    <div className="px-3 py-2">
                      <span className="block text-[16px] text-slate-400 uppercase tracking-wider mb-0.5">วันทำงาน</span>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{periodStats?.netWorkDays} <span className="text-[9px] font-normal text-slate-400">วัน</span></span>
                    </div>
                    <div className="px-3 py-2">
                      <span className="block text-[16px] text-slate-400 uppercase tracking-wider mb-0.5">ส-อา</span>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{(periodStats?.totalSaturdays || 0) + (periodStats?.totalSundays || 0)} <span className="text-[9px] font-normal text-slate-400">วัน</span></span>
                    </div>
                    <div className="px-3 py-2">
                      <span className="block text-[16px] text-slate-400 uppercase tracking-wider mb-0.5">นักขัตฤกษ์ (จ-ศ)</span>
                      <button
                        onClick={() => setActiveTab('holidays')}
                        className="text-sm font-bold text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 underline decoration-dotted underline-offset-2 transition-all text-left"
                      >
                        {periodStats?.holidaysOnWorkDays} <span className="text-[9px] font-normal">วัน</span>
                      </button>
                    </div>
                    <div className="px-3 py-2">
                      <span className="block text-[16px] text-slate-400 uppercase tracking-wider mb-0.5">รวม</span>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200">
                        {periodStats?.totalWeeks} สัปดาห์ {periodStats?.remainingDays} วัน
                        <span className="block text-[18px] font-normal text-slate-400">({periodStats?.totalDays} วัน)</span>
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="px-4 py-2 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group"
                      >
                        ...
                        <div className="flex flex-wrap xl:flex-nowrap items-center gap-2">
                          ...
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <input
                              ...
                              className="flex-1 min-w-[100px] text-xs font-semibold bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-indigo-500 dark:focus:border-indigo-400 outline-none py-1 text-slate-900 dark:text-slate-100 placeholder:text-slate-300 dark:placeholder:text-slate-600"
                            />
                            <select
                              value={row.province}
                              onChange={(e) => {
                                const p = e.target.value;
                                updateRow(row.id, { province: p, minWage: wages[p] || 0 });
                              }}
                              className="text-xs font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1.5 py-1 outline-none focus:ring-1 focus:ring-indigo-500 dark:text-slate-100 w-28 shrink-0"
                            >
                              <option value="" disabled hidden>จังหวัด</option>
                              {PROVINCES.map(p => <option key={p} value={p}>{p}</option>)}
                            </select>
                            <select
                              value={row.selectedProfileId}
                              onChange={(e) => updateRow(row.id, { selectedProfileId: e.target.value })}
                              className="text-xs font-semibold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/50 rounded px-1.5 py-1 outline-none focus:ring-1 focus:ring-indigo-500 w-44 shrink-0"
                            >
                              <option value="" disabled hidden>รูปแบบผลัด</option>
                              {profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                            </select>
                            <div className="flex items-center gap-1 shrink-0">
                              <span className="text-[10px] text-slate-400">จุด</span>
                              <input
                                type="number" min="1"
                                value={row.points}
                                onChange={(e) => updateRow(row.id, { points: Number(e.target.value) })}
                                className="w-14 text-xs font-mono font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1.5 py-1 outline-none focus:ring-1 focus:ring-indigo-500 dark:text-slate-100 text-center"
                              />
                            </div>
                          </div>

                          {/* Divider */}
                          <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 shrink-0 hidden xl:block" />

                          {/* Computed stats */}
                          <div className="flex items-center gap-3 shrink-0">
                            <div className="text-center">
                              <div className="text-[12px] text-slate-400 leading-none mb-0.5">ค่าแรง/วัน</div>
                              <input
                                type="number"
                                value={row.minWage}
                                onChange={(e) => updateRow(row.id, { minWage: Number(e.target.value) })}
                                className="w-14 text-[10px] font-mono font-bold bg-slate-100 dark:bg-slate-800 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-indigo-500 dark:text-slate-100 text-center"
                              />
                            </div>
                            <div className="text-center">
                              <div className="text-[12px] text-slate-400 leading-none mb-0.5">ปกติ</div>
                              <span className="text-[10px] font-mono font-semibold text-slate-700 dark:text-slate-300">{(budget?.totalShiftsNormal || 0).toLocaleString()}</span>
                            </div>
                            <div className="text-center">
                              <div className="text-[12px] text-slate-400 leading-none mb-0.5">หยุด</div>
                              <span className="text-[10px] font-mono font-semibold text-slate-700 dark:text-slate-300">{(budget?.totalShiftsHoliday || 0).toLocaleString()}</span>
                            </div>
                            <div className="text-center">
                              <div className="text-[12px] text-slate-400 leading-none mb-0.5">ฐาน/จุด</div>
                              <span className="text-[10px] font-mono font-semibold text-emerald-600 dark:text-emerald-400">฿{budget?.baseTotal.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                            </div>
                            <div className="text-center hidden xl:block">
                              <div className="text-[12px] text-slate-400 leading-none mb-0.5">สปส.+บริหาร</div>
                              <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">฿{((budget?.socialSecurity || 0) + (budget?.managementFee || 0)).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                            </div>
                          </div>

                          {/* Grand total + delete */}
                          <div className="flex items-center gap-1.5 shrink-0 ml-auto">
                            <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-lg px-3 py-1.5 text-right min-w-[120px]">
                              <div className="text-[9px] font-bold text-indigo-500 dark:text-indigo-400 leading-none mb-0.5">รวม {row.points} จุด</div>
                              <div className="text-sm font-black text-indigo-700 dark:text-indigo-300 leading-none">฿{budget?.grandTotal.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</div>
                            </div>
                            <button
            ...
            {rows.length > 0 && rows.some(r => calculateBudget(r) !== null) && (
              <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                ...
                {/* Always visible Footer/Summary Row */}
                <div className="border-t-2 border-indigo-100 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-900/20 px-4 py-3 flex items-center justify-between">
                  <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase tracking-wider">รวมทั้งหมด</span>
                  <div className="flex items-center gap-12">
                    <div className="text-center font-mono font-bold text-indigo-700 dark:text-indigo-300">
                      {rows.reduce((acc, r) => acc + r.points, 0)} จุด
                    </div>
                    <div className="text-right font-mono font-black text-lg text-indigo-700 dark:text-indigo-300">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="space-y-1 text-center md:text-left">
                    ...
                    <h3 className="text-4xl md:text-5xl font-black tracking-tighter">
                      ฿ {(rows.reduce((acc, row) => acc + (calculateBudget(row)?.grandTotal || 0), 0) * 1.07).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </h3>
                    <p className="text-indigo-200 text-xs font-bold uppercase tracking-widest mt-1 opacity-80">(รวม VAT 7% แล้ว)</p>
                  </div>
          ...
          // Pre-calculate holidays for the entire range shown in the tab
          const startYear = parse(startDate, 'yyyy-MM', new Date()).getFullYear();
          const endYear = parse(endDate, 'yyyy-MM', new Date()).getFullYear();

          const allBaseHolidays = [...customHolidays];
          // Ensure standard holidays for relevant years are included to correctly identify user custom ones
          for (let y = startYear - 1; y <= endYear + 1; y++) {
            DEFAULT_THAI_HOLIDAYS(y).forEach(sh => {
              if (!allBaseHolidays.some(h => h.date === sh.date)) {
                allBaseHolidays.push(sh);
              }
            });
          }

          const effectiveHolidays = getEffectiveHolidays(allBaseHolidays);
          const standardDates = new Set();
          for (let y = startYear - 1; y <= endYear + 1; y++) {
            DEFAULT_THAI_HOLIDAYS(y).forEach(sh => standardDates.add(sh.date));
          }
            ...
            >
              <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {months.map((month) => {
                    ...
                    const firstDayEmptyCells = monthStart.getDay();
                      ...
                      <div key={`${month.month}-${month.year}`} className="space-y-4">
                        ...
                        <div className="grid grid-cols-7 gap-1">
                          ...
                          {daysInMonth.map(day => {
                            ...
                            const isStandardHoliday = holiday && standardDates.has(holiday.date);
                            const isSubstituteHoliday = holiday && !isStandardHoliday && holiday.name.startsWith('ชดเชย');
                            const isUserCustomHoliday = holiday && !isStandardHoliday && !isSubstituteHoliday;
                            ...
                            return (
                              ...
                              >
                                ...
                                {isStandardHoliday && <div className="w-1 h-1 bg-red-400 rounded-full mt-0.5" />}
                                {isSubstituteHoliday && <div className="w-1 h-1 bg-violet-400 rounded-full mt-0.5" />}
                                {isUserCustomHoliday && <div className="w-1 h-1 bg-amber-400 rounded-full mt-0.5" />}
                              </button>
                            );
                          })}
                        </div>
              ...
              <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                  <h2 className="font-bold text-slate-800 dark:text-slate-100">รายการวันหยุดทั้งหมด</h2>
                </div>
                <div className="p-0 overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20">
                        <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider w-12">#</th>
                        <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider w-32">วันที่</th>
                        <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider">ชื่อวันหยุด</th>
                        <th className="px-4 py-2 text-center text-[10px] font-bold text-slate-500 uppercase tracking-wider w-20">จัดการ</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                      {effectiveHolidays
                        ...
                        .map((h, idx) => {
                          const isStandardHoliday = standardDates.has(h.date);
                          return (
                            <tr key={h.date} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                              <td className="px-4 py-1.5 text-xs font-mono text-slate-400">{idx + 1}</td>
                              <td className="px-4 py-1.5 font-mono text-xs text-slate-600 dark:text-slate-400">
                              ...
                              <td className="px-4 py-1.5">
                                <span className={cn(
                                  ...
                                  h.name.startsWith('ชดเชย')
                                    ? "bg-violet-50 dark:bg-violet-900/20 text-violet-600 dark:text-violet-400 border border-violet-100 dark:border-violet-800/50"
                                    : isStandardHoliday
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  คุณต้องการนำเข้าข้อมูลรูปแบบใด?
                </p>
                <div className="grid grid-cols-1 gap-3">
                  <button
                    onClick={() => applyImport('overwrite')}
                    className="flex flex-col items-start p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-all group"
                  >
                    <span className="font-bold text-slate-800 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">เขียนทับ (Overwrite)</span>
                    <span className="text-xs text-slate-500 dark:text-slate-400">ล้างข้อมูลเดิมและแทนที่ด้วยข้อมูลใหม่ทั้งหมด</span>
                  </button>
                  <button
                    onClick={() => applyImport('merge')}
                    className="flex flex-col items-start p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-emerald-500 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/20 transition-all group"
                  >
                    <span className="font-bold text-slate-800 dark:text-slate-100 group-hover:text-emerald-600 dark:group-hover:text-emerald-400">เพิ่มต่อ (Merge)</span>
                    <span className="text-xs text-slate-500 dark:text-slate-400">เพิ่มข้อมูลใหม่ต่อจากข้อมูลเดิมที่มีอยู่</span>
                  </button>
                </div>
              </div>
      ...
      <AnimatePresence>
        {holidayImportModal?.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 dark:text-slate-100">ยืนยันการนำเข้าวันหยุด</h3>
                <button onClick={() => setHolidayImportModal(null)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 p-4 rounded-xl">
                  <div className="flex gap-3">
                    ...
                    <p className="text-sm text-amber-800 dark:text-amber-200">
                      พบวันหยุดพิเศษ {holidayImportModal.holidays.length} รายการในไฟล์ คุณต้องการบันทึกวันหยุดเหล่านี้ลงในฐานข้อมูลส่วนกลาง (Shared Database) หรือไม่?
                    </p>
                  </div>
                </div>

                <div className="max-h-40 overflow-y-auto border border-slate-100 dark:border-slate-800 rounded-xl divide-y divide-slate-50 dark:divide-slate-800">
                  {holidayImportModal.holidays.map((h, i) => (
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 italic text-center">
                  * หากคุณเลือก "ไม่บันทึก" วันหยุดจะถูกนำมาใช้สำหรับการคำนวณในครั้งนี้เท่านั้น แต่จะไม่ถูกบันทึกถาวรลงฐานข้อมูล
                </p>
              </div>
              <div className="p-6 bg-slate-50 dark:bg-slate-800/50 flex flex-col gap-3">
                <button
                  onClick={() => confirmHolidayImport(true)}
                  className="w-full px-4 py-3 text-sm font-bold text-white bg-indigo-600 dark:bg-indigo-500 rounded-xl hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-all shadow-lg shadow-indigo-100 dark:shadow-none"
                >
                  ยืนยัน และบันทึกเข้า Database
                </button>
                <div className="flex gap-3">
                  <button
                    onClick={() => confirmHolidayImport(false)}
                    className="flex-1 px-4 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all"
                  >
                    นำเข้าโดยไม่บันทึก
                  </button>
                  <button
                    onClick={() => setHolidayImportModal(null)}
                    className="flex-1 px-4 py-2.5 text-sm font-bold text-slate-400 dark:text-slate-500 bg-transparent rounded-xl hover:bg-slate-200 dark:hover:bg-slate-800 transition-all"
                  >
                    ยกเลิกทั้งหมด
                  </button>
                </div>
