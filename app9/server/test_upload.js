...
async function run() {
  try {
    ...
    let count = 0;
    const row = data[1548]; // 10.223.164.135 should be somewhere here
    ...
    console.log("NE_IP:", ne_ip);
    console.log("Values:", values);

    const res = await pool.query(query, values);
    console.log("Insert result:", res.rowCount);
    
    const verify = await pool.query('SELECT * FROM olt_base_data WHERE ne_ip = $1', [ne_ip]);
    console.log("DB Data:", verify.rows[0]);
