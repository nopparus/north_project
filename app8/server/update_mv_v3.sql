...
-- 4. All unique circuits across all tables (ONU Records ONLY)
all_circuits AS (
...
final_assembly AS (
    SELECT
        ...
        wf.wifi_max_speed,
        CASE WHEN s.has_olt THEN olt.olt_raw_name ELSE onu.onu_raw_name END as onu_raw_name,
        s.has_onu, s.has_olt, s.has_wifi,
        CASE
            WHEN (CASE WHEN s.has_olt THEN olt.olt_type ELSE onu.onu_type END ILIKE '%all in one%' OR CASE WHEN s.has_olt THEN olt.olt_type ELSE onu.onu_type END ILIKE '%all-in-one%')
                 AND wf.circuit_norm IS NOT NULL
                 AND wf.wifi_max_speed IS NOT NULL AND wf.wifi_max_speed != ''
                 AND CASE WHEN s.has_olt THEN olt.olt_max_speed ELSE onu.onu_max_speed END IS NOT NULL AND CASE WHEN s.has_olt THEN olt.olt_max_speed ELSE onu.onu_max_speed END != ''
            THEN 
                 CASE 
                     WHEN COALESCE(CAST(NULLIF(REGEXP_REPLACE(CASE WHEN s.has_olt THEN olt.olt_max_speed ELSE onu.onu_max_speed END, '[^0-9.]', '', 'g'), '') AS NUMERIC), 0) > 
                          COALESCE(CAST(NULLIF(REGEXP_REPLACE(wf.wifi_max_speed, '[^0-9.]', '', 'g'), '') AS NUMERIC), 0)
                     THEN CASE WHEN s.has_olt THEN olt.olt_max_speed ELSE onu.onu_max_speed END
                     ELSE wf.wifi_max_speed
                 END
            WHEN wf.wifi_max_speed IS NOT NULL AND wf.wifi_max_speed != '' THEN wf.wifi_max_speed
            WHEN (CASE WHEN s.has_olt THEN olt.olt_lan_fe ELSE onu.lan_fe END IS NOT NULL AND CASE WHEN s.has_olt THEN olt.olt_lan_fe ELSE onu.lan_fe END != '' AND CASE WHEN s.has_olt THEN olt.olt_lan_fe ELSE onu.lan_fe END != '0')
                 AND (CASE WHEN s.has_olt THEN olt.olt_lan_ge ELSE onu.lan_ge END IS NULL OR CASE WHEN s.has_olt THEN olt.olt_lan_ge ELSE onu.lan_ge END = '' OR CASE WHEN s.has_olt THEN olt.olt_lan_ge ELSE onu.lan_ge END = '0')
                 THEN '100 Mbps (FE Only)'
            ELSE CASE WHEN s.has_olt THEN olt.olt_max_speed ELSE onu.onu_max_speed END
        END as effective_max_speed,
        CASE 
            WHEN wf.circuit_norm IS NULL 
                 AND NOT (COALESCE(CASE WHEN s.has_olt THEN olt.olt_type ELSE onu.onu_type END, '') ILIKE '%all in one%')
