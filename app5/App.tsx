...
import { NAVIGATION, MOCK_RECORDS, MOCK_PROJECTS, ANNUAL_SCHEDULE } from './constants';
import { Menu, X, Check, Briefcase, Palette } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const validProjects = projects.filter(p => p.workType === workMode);
  ...
  const [scheduleItems, setScheduleItems] = useState<ScheduleItem[]>(ANNUAL_SCHEDULE);
  ...
  const handleUpdateScheduleItems = (updatedItems: ScheduleItem[]) => {
    setScheduleItems(prev => {
      const otherProjectItems = prev.filter(item => item.projectId !== selectedProjectId);
      return [...otherProjectItems, ...updatedItems];
    });
  };
  ...
  const handleUpdateProjectCategories = (categoryIds: string[]) => {
    setProjects(prev => prev.map(p => 
      p.id === selectedProjectId ? { ...p, equipmentTypes: categoryIds } : p
    ));
  };
  ...
  const handleAddProject = (e: React.FormEvent) => {
    ...
    const projectToAdd: Project = {
      ...
      color: workMode === 'PM' ? 'bg-blue-600' : 'bg-emerald-600'
    };
    setProjects(prev => [...prev, projectToAdd]);
    setSelectedProjectId(id);
    setIsProjectModalOpen(false);
    setNewProject({ name: '', status: 'active', color: workMode === 'PM' ? 'bg-blue-600' : 'bg-emerald-600' });
  };
  ...
  const renderContent = () => {
    if (!currentProject) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-slate-400">
           ...
           <button onClick={() => setIsProjectModalOpen(true)} className="mt-4 text-blue-600 font-black uppercase text-xs hover:underline">สร้างโครงการใหม่ในหมวดนี้</button>
        </div>
      );
    }
    ...
    switch (activeTab) {
      case 'dashboard':
        return <Dashboard records={projectRecords} currentProject={currentProject} workMode={workMode} onAction={(loc, eq) => {
      ...
      case 'schedule':
        return (
          <Schedule 
            ...
            workMode={workMode} // ส่ง workMode ไปด้วยเพื่อปรับ UI
          ...
          <div className="p-4 md:p-8 text-white">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
               ...
               <div className="flex items-center space-x-2">
                 ...
                 <span className={`px-4 py-1.5 rounded-full text-[10px] font-black text-white shadow-lg ${currentProject.color}`}>{currentProject.status.toUpperCase()}</span>
               </div>
            </div>
      ...
      <main className="flex-1 overflow-y-auto flex flex-col w-full">
        <header className="bg-slate-900/50 backdrop-blur-md border-b border-slate-800 px-4 md:px-8 py-3 sticky top-0 z-30 flex justify-between items-center h-16 shadow-2xl">
          <div className="flex items-center space-x-3 md:space-x-4">
            <button onClick={toggleSidebar} className="p-2 hover:bg-slate-800 rounded-xl text-slate-400 transition-all"><Menu size={24} /></button>
            <div className="flex flex-col">
              <h1 className="text-sm md:text-base font-black text-white tracking-tight leading-none uppercase">
                {NAVIGATION.find(n => n.id === activeTab)?.name}
              </h1>
              <span className="text-[10px] text-blue-400 font-black mt-1 uppercase tracking-widest truncate max-w-[150px] md:max-w-md">PROJECT: {currentProject?.name || 'No Project Selected'}</span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-x-hidden"><div className="container mx-auto pb-10">{renderContent()}</div></div>
      </main>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
           <div className="bg-slate-900 border border-slate-700 rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-slate-800 p-8 text-white relative border-b border-slate-700">
               ...
               <button onClick={() => setIsProjectModalOpen(false)} className="absolute top-6 right-6 p-2 hover:bg-white/10 rounded-xl transition-colors"><X size={24} /></button>
            </div>
            <form onSubmit={handleAddProject} className="p-8 space-y-6">
               ...
               <button type="submit" className={`w-full text-white py-4 rounded-2xl font-black uppercase shadow-xl transition-all ${workMode === 'PM' ? 'bg-blue-600 hover:bg-blue-700 shadow-blue-200' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200'}`}>
